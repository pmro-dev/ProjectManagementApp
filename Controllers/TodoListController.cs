...
namespace TODO_List_ASPNET_MVC.Controllers
{
	...
	public class TodoListController : Controller
	{
		...
		private const int IndexForSort = 0;
		...
		public TodoListController(IContextOperations context, SignInManager<IdentityUser> signInManager, ILogger<TodoListController> logger)
		{
			_context = context;
			_logger = logger;
		...
		{
			operationName = OtherHelp.GetActionNameForLoggingAndExceptions(nameof(Briefly), controllerName);
		...
		{
			operationName = OtherHelp.GetActionNameForLoggingAndExceptions(nameof(All), controllerName);
		...
		{
			operationName = OtherHelp.GetActionNameForLoggingAndExceptions(nameof(SingleDetails), controllerName);
			...
			TodoListViewModel todoListViewModel = new()
			{
				...
				TasksForToday = allTodoListTasks.Where(t => t.DueDate.ToShortDateString() == todayDate.ToShortDateString()).ToList(),
			...
			todoListViewModel.TasksNotCompleted.Sort(IndexForSort, todoListViewModel.TasksNotCompleted.Count, new TasksComparer());
			todoListViewModel.TasksForToday.Sort(IndexForSort, todoListViewModel.TasksForToday.Count, new TasksComparer());
			todoListViewModel.TasksCompleted.Sort(IndexForSort, todoListViewModel.TasksCompleted.Count, new TasksComparer());
		...
		public async Task<IActionResult> Create([Bind("Name, UserId")] TodoListModel todoListModel)
		{
			...
			{
				if (await _context.DoesTodoListWithSameNameExist(todoListModel.Name))
				{
				...
				todoListModel.UserId = _signInManager.UserManager.GetUserId(User);
