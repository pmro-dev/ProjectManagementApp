...
public class DatabaseOperationsTests : BaseOperationsSetup
{
	...
	private static TaskModel PrepareTask(string taskTitle, string taskDescription, DateTime taskDueDate)
	{
		...
		var temp = TasksCollection[_lastIndex].Id.ToString();
		temp = temp.Remove(temp.Length - 1, _valueOneIndicator);
		temp += "1";
	...
	{
		Guid taskIdForMockSetup = TasksCollection[_firstIndex].Id;
	...
	{
		Guid taskToUpdateId = TasksCollection[_firstIndex].Id;
